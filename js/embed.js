...
$(document).ready(function(){ 
	...
	// get screen size
	rtiDataWidth = $(document).width();
	rtiDataHeight = $(document).height();
	...
	if( rtiFolder === null ) {
		...
		rti = createRtiViewer(rtiDataContainer, rtiDataPath, rtiDataWidth, rtiDataHeight);
		rti.setMode(1);
	...
	var screenResizeFunction = debounce(function() {
		// The function's code
		...
		canvas.width(rtiDataWidth);
		canvas.height(rtiDataHeight);
		canvas.attr("width", rtiDataWidth);
		canvas.attr("height", rtiDataHeight);
		console.log(rtiDataWidth + "x"  + rtiDataHeight);
		rti.resize();

	}, 250);
	
	window.addEventListener('resize', screenResizeFunction);
...
function makeSiblingThumbnails() {
	...
	for( var s=0; s<siblings.length; s++ ) {
		...
		extrasHtml += '    <a href="embed.html?obj=' + siblings[s] + '" title="' + type + '">';
